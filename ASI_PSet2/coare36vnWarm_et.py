...
"""
NOTES FOR GRADING BY DR. SEO:
The major edit in this file is allowing the passing of the mode control (zo_mode) and the newly required units (Cm) and (Theta) into the main calculation loop. The edits are on lines: 30, 138, 207-210, 313, and 339
...
def coare36vnWarm_et(Jd, U, Zu, Tair, Zt, RH, Zq, P, Tsea, SW_dn, LW_dn, Lat, Lon, Zi, Rainrate, Ts_depth, Ss, cp=None, sigH=None,zrf_u = 10.0,zrf_t = 10.0,zrf_q = 10.0, zo_mode=None, Cm=None, Theta=None): 
    print('WarmCoolLayer')
    print(Theta)
#***********   input data **************
...
# Call coare36vn to get initial flux values
    Bx = c36.coare36vn_zrf_et(U[0],Zu[0],Tair[0],Zt[0],RH[0],Zq[0],P[0],Tsea[0],SW_dn[0],LW_dn[0],Lat[0],Lon[0],Jd[0],Zi[0],Rainrate[0],Ss[0],cp[0],sigH[0],zrf_u,zrf_t,zrf_q,zo_mode,Cm[0],Theta[0])
    ...
    for ibg in np.arange(0,nx).reshape(-1):  #np.arange(0,53).reshape(-1):
        ...
        '''
        This code pulls indivudal values based on the index from the arrays passed into the function
        '''
        cm=Cm[ibg]
        theta=Theta[ibg]
        ...
        Bx = c36.coare36vn_zrf_et(u,zu,t,zt,rh,zq,p,ts,sw_dn,lw_dn,lat,lon,jd,zi,rain,ss,cpi,sigHi,zrf_u,zrf_t,zrf_q,zo_mode,cm,theta)
    ...
    Bx = c36.coare36vn_zrf_et(U,Zu,Tair,Zt,RH,Zq,P,Tsea,SW_dn,LW_dn,Lat,Lon,Jd,Zi,Rainrate,Ss,cp,sigH,zrf_u,zrf_t,zrf_q,zo_mode,Cm,Theta)
