...
All of my below comments follow the ''' format. The critical thing I added was an additional nonrequired
input called zo_mode that must be set to either 12 or 13 to trigger the new formulation. The new chunk of
code lives between lines 608 and 644
...
def coare36vn_zrf_et(u, zu , t, zt, rh, zq, P, ts, sw_dn, lw_dn, lat, lon,jd, zi,rain, Ss, cp=None, sigH=None, zrf_u=10.0, zrf_t=10.0, zrf_q=10.0, zo_mode=None, cm=None, theta=None):   
...
# Best to avoid NaNs as inputs as well. Will prevent weird results
    ...
    '''
    Code put in to allow the entry of significant wave height and dominant phase speed
    '''
...
# input us = 0*u here;
    ...
    for i in np.arange(1,nits+1).reshape(-1):
        ...
        '''
        KADE
        Below is the code I wrote myself to add the other forms of charnock coefficent, 
        I used a zo_mode as an input that allows switching between formulations
        '''
        ...
        if zo_mode == 12:
            ...
            D13 = 0.09
            B13 = 2
            valid = (~np.isnan(cp)) & (~np.isnan(sigH)) & (cp > 0)
            
            zo13 = sigH * D13 * np.cos(a13 * theta) * (usr / cp) ** (B13 * np.cos(b13 * theta))
